...
public class AvitoVisitorBotService {
    ...
    private String visitAndCapture(Robot robot, Apartment apt) throws Exception {
        ...
        chord(robot, KeyEvent.VK_CONTROL, KeyEvent.VK_U); // "Просмотр кода страницы" — открывает новую вкладку
        sleep(randomBetween(VIEW_SOURCE_LOAD_MIN_MS, VIEW_SOURCE_LOAD_MAX_MS));

        chord(robot, KeyEvent.VK_CONTROL, KeyEvent.VK_A); // выделить весь текст исходника
        sleep(randomBetween(200, 500));
        chord(robot, KeyEvent.VK_CONTROL, KeyEvent.VK_C); // скопировать в буфер обмена
    ...
    private void closeTab(Robot robot) {
        chord(robot, KeyEvent.VK_CONTROL, KeyEvent.VK_W); // macOS: VK_META
    }

    private void chord(Robot robot, int modifier, int key) {
